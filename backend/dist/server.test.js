...
describe('User Management Endpoints', () => {
    ...
    beforeEach(() => {
        mockJwtVerify.mockReturnValue({ userId: 'test-user-id', phone_number: '+1234567890' });
    });
    ...
    describe('GET /users/me/trips', () => {
        ...
        it('should filter trips by status', async () => {
            ...
            const response = await request(app)
                ...
                .set('Authorization', `Bearer ${mockToken}`)
                .expect(200);
            expect(testDb.query).toHaveBeenCalledWith(expect.stringContaining("status = 'confirmed'"), expect.any(Array));
        });
    });
});
describe('Villa Management Endpoints', () => {
    ...
    beforeEach(() => {
        mockJwtVerify.mockReturnValue({ userId: 'host-001', phone_number: '+1234567890' });
    });
    describe('GET /villas', () => {
...
describe('Booking Management Endpoints', () => {
    ...
    beforeEach(() => {
        mockJwtVerify.mockReturnValue({ userId: 'guest-001', phone_number: '+1234567890' });
    });
    ...
    describe('GET /bookings', () => {
        ...
        it('should filter bookings by status', async () => {
            ...
            const response = await request(app)
                ...
                .set('Authorization', `Bearer ${mockToken}`)
                .expect(200);
            expect(testDb.query).toHaveBeenCalledWith(expect.stringContaining("status = 'pending'"), expect.any(Array));
        });
    });
});
describe('Messaging System', () => {
    ...
    beforeEach(() => {
        mockJwtVerify.mockReturnValue({ userId: 'user-001', phone_number: '+1234567890' });
    });
...
describe('WebSocket Events', () => {
    ...
    describe('message_received event', () => {
        it('should broadcast message to recipient', async () => {
            ...
            expect(mockClient.send).toHaveBeenCalledWith(expect.stringContaining('message_received'));
        });
    });
    describe('booking_status_updated event', () => {
        it('should notify guest and host of status change', async () => {
            ...
            expect(mockGuestClient.send).toHaveBeenCalledWith(expect.stringContaining('booking_status_updated'));
            expect(mockHostClient.send).toHaveBeenCalledWith(expect.stringContaining('booking_status_updated'));
        });
    });
    describe('user_typing event', () => {
        it('should broadcast typing indicator', async () => {
            ...
            expect(mockClient.send).toHaveBeenCalledWith(expect.stringContaining('user_typing'));
        });
    });
});
describe('Error Handling', () => {
    ...
    describe('Authorization Errors', () => {
        it('should prevent guest from accessing host endpoints', async () => {
            mockJwtVerify.mockReturnValue({ userId: 'guest-001' });
            testDb.query.mockResolvedValueOnce({
...
describe('Performance Tests', () => {
    describe('Rate Limiting', () => {
        it('should handle multiple concurrent requests', async () => {
            const promises = Array(10).fill(null).map(() => request(app).get('/api/villas'));
    ...
    describe('Pagination', () => {
        it('should limit results per page', async () => {
            ...
            expect(testDb.query).toHaveBeenCalledWith(expect.stringContaining('LIMIT 10 OFFSET 0'), expect.any(Array));
        });
    });
});
describe('Integration Tests', () => {
    describe('Complete Booking Flow', () => {
        it('should handle full booking lifecycle', async () => {
            ...
            mockJwtVerify.mockReturnValue({ userId: 'host-001' });
            testDb.query.mockResolvedValueOnce({ rows: [mockBooking] });
            testDb.query.mockResolvedValueOnce({ rows: [mockVilla] });
            testDb.query.mockResolvedValueOnce({
                rows: [{ ...mockBooking, status: 'confirmed' }]
    ...
    describe('Complete Messaging Flow', () => {
        it('should handle message exchange between users', async () => {
            ...
            mockJwtVerify.mockReturnValue({ userId: 'user-002' });
            testDb.query.mockResolvedValueOnce({ rows: [mockUser2] });
            testDb.query.mockResolvedValueOnce({ rows: [mockMessage] });
